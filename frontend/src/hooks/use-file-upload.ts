...
export const useFileUpload = (
  ...
  const handleDrop = useCallback((e: DragEvent<HTMLElement>) => { e.preventDefault(); e.stopPropagation(); setState((prev) => ({ ...prev, isDragging: false })); if (inputRef.current?.disabled) return; if (e.dataTransfer.files?.length) { if (!multiple) { addFiles([e.dataTransfer.files[0]]) } else { addFiles(e.dataTransfer.files) } } }, [addFiles, multiple])
  ...
  const getInputProps = useCallback((props: InputHTMLAttributes<HTMLInputElement> = {}) => ({ ...props, type: "file" as const, onChange: handleFileChange, accept: props.accept || accept, multiple: props.multiple !== undefined ? props.multiple : multiple, ref: inputRef as any }), [accept, multiple, handleFileChange])
